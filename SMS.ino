...
void SMS_Del_All(void){
 unsigned int i,j;
       ...
       for(pV=0;pV<100 & pV<len;pV++) Soft_uart_send( pgm_read_byte_near (ATQMGDA+pV));          
         ...
         for(i=0;i<8430;i++){if(mySerial.available())gsm_data[recv_cnt++] = mySerial.read(); delay(1);  if(recv_cnt>148)recv_cnt=147;}
        print_strU0("Dell " );
...
void Check_RecievedSMS(unsigned char Val){
   ...
   if(Val == 100){Val=2;  
     SoftUartNL();}
   for(i=0;i<(55000*Val);i++){if(mySerial.available()){gsm_data[recv_cnt++] = mySerial.read(); i=0; }  if(recv_cnt>248)recv_cnt=247;}
